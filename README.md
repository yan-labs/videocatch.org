...
Nothing is uploaded to VideoCatch, nothing is stored, and there is no account. Your browser fetches the media from the platform's CDN; for a few CDNs that refuse browser downloads, VideoCatch streams the file through without keeping a copy.
...
- **No storage.** Files never sit on a VideoCatch server. Cached metadata expires within an hour.
- **No account, no tracking beyond aggregate counts.** See the [privacy policy](https://videocatch.org/privacy).
